# AWS Certified Cloud Practitioner (ACCP) Exam Cheatsheet
...
Official Site [link](https://aws.amazon.com/certification/certified-cloud-practitioner/) 
...
### Table of contents
 ...
 - 2) [Six advantages of Amazon Cloud (Benefits)](#six-advantages-of-amzon-cloud-benefits)
 - 3) [AWS Compute Optimizer](#aws-compute-optimizer)
 - 4) [Amazon EBS volumes types](#amazon-ebs-volumes-types)
 - 5) [Amazon load balancers](#amazon-load-balancers)
 - 6) [AWS Security Groups](#aws-security-groups)
 - 7) [AWS Internet Gateway](#aws-internet-gateway)
 - 8) [AWS Scalability](#aws-scalability)
 - 9) [Amazon Simple Notification Service (SNS)](#amazon-simple-notification-service-sns)
 - 10) [Amazon Simple Queue Service (SQS)](#amazon-simple-queue-service-sqs)
 - 11) [AWS Inspector](#aws-inspector)
 - 12) [AWS Trusted Advisor](#aws-trusted-advisor)
 - 13) [AWS Personal Health Dashboard](#aws-personal-health-dashboard)
 - 14) [AWS TCO Calculator](#aws-tco-calculator)
 - 15) [AWS EBS](#aws-ebs)
 - 16) [Amazon SageMaker](#amazon-sagemaker)
 - 17) [Amazon Kinesis](#amazon-kinesis)
 - 18) [Cloud Computing](#cloud-computing)
 - 19) [AWS CloudFormation](#aws-cloudformation)
 - 20) [AWS Elastic Beanstalk](#aws-elastic-beanstalk)
 - 21) [AWS Lambda](#aws-lambda)
 - 22) [AWS Well-Architected Framework](#aws-well-architected-framework)
 - 23) [AWS WAF](#aws-waf)
 - 24) [Amazon Kinesis](#amazon-kinesis)
 - 25) [AWS AD Connector](#aws-ad-connector)
 - 26) [AWS Simple AD](#aws-simple-ad)
 - 27) [Amazon Elastic Container Service for Kubernetes (EKS)](#amazon-elastic-container-service-for-kubernetes-eks)
 - 28) [Amazon Elastic Container Service (ECS)](#amazon-elastic-container-service-ecs)
 - 29) [Virtual Private Gateway](#virtual-private-gateway)
 - 30) [VPC peering connection](#vpc-peering-connection)
 - 31) [AWS support plans](#aws-support-plans)
 - 32) [Amazon IAM](#amazon-iam)
 - 33) [Popular HTTP code](#popular-htttp-code)
 - 34) [AWS CloudTrail](#aws-cloudtrail)
 - 35) [Amazon DynamoDB](#amazon-dynamodb)
 - 36) [AWS Database SQL type](#aws-database-sql-type)
 - 37) [AWS Device Farm](#aws-device-farm)
 - 38) [AWS Config](#aws-config)
 - 39) [AWS Shared Responsibility Model](#aws-shared-responsibility-model)
 - 40) [Amazon RDS](#amazon-rds)
 - 41) [AWS Auto Scaling Group](#aws-auto-scaling-group)
 - 42) [AWS CloudHSM](#aws-cloudhsm)
 - 43) [AWS Auto Scaling](#aws-auto-scaling)
 - 44) [AWS Glacier](#aws-glacier)
 - 45) [AWS Storage Gateway Volume Gateway](#aws-storage-gateway-volume-gateway)
 - 46) [AWS Step Functions](#aws-step-functions)
 - 47) [Amazon Simple Workflow Service (SWF)](#amazon-simple-workflow-service-swf)
 - 48) [Amazon Security Token Service (STS)](#amazon-security-token-service-sts)
 - 49) [AWS Server Migration Service (SMS)](#aws-server-migration-service-sms)
 - 50) [Amazon DevPay](#amazon-devpay)
 - 51) [Amazon Elasticsearch Service](#amazon-elasticsearch-service)
 - 52) [Amazon Athena](#amazon-athena)
 - 53) [Amazon QuickSight](#amazon-quicksight)
 - 54) [Amazon CodeStar](#amazon-codestar)
 - 55) [Amazon Cloud9](#amazon-cloud9)
...
- Amazon S3 storage tier:
	- **S3 Standard** -> 99.99% Service Level Agreements (SLA) -> for data that is accessed less frequently but requires rapid access when needed
	- **S3 Standard-IA(Infrequent Access)** -> 99.9% Service Level Agreements (SLA) -> offers the high durability, high throughput, and low latency of S3 Standard
	- **S3 One Zone-IA(Infrequent Access)** -> 99% Service Level Agreements (SLA) ->  *the most cost-effective* Amazon S3 storage tier for data that is not often accessed but requires high durability. It stores data in a **single** AZ
	- **Glacier** -> No Service Level Agreements (SLA)
...
- **Provisioned IOPS(io1)**(SSD): is a high-performance volume type that is more expensive and should be used for apps that require higher performance.
...
- IAM **supported authentication** methods include:
	...
	- access keys
	- server certificates
- Best practice to ensure the security of the AWS account
...
- You can scale the DB anytime **without incurring downtime**.
...
### AWS Config
-  It allows you to **automate the evaluation of recorded** configurations against desired configurations.
...
- AWS is responsible for **compute infrastructure**.
...
- **Shared Controls**: Apply to both the *infrastructure layer* and *customer layers*
	...
	- Configuration Management: AWS -> Configuration of its infra devices | Customer -> Configuration of their OS, Apps, DBs.
...
- helps developers build, run, and scale background jobs with parallel or sequential steps.
...
- It's an **agentless** service, which makes it easier and faster for you to migrate on-premises workloads to AWS.
- You can migrate Virtual Machines from **VMware vSphere** and **Windows Hyper-V** to AWS with this service.

### Amazon DevPay
- That makes it easy for businesses to **sell applications** that are built in or run on top of, *Amazon Web Services*.
...
### Amazon Lightsail
- It provides developers with computing, storage, and networking capacity and capabilities to deploy and manage websites, web applications, and databases in the cloud. Also, it provides **preconfigured VPS** that includes **everything required to deploy** or create a **DB**.
...
- Types of Reserved Instance(RI):
	...
	- Payment options for reserved instances include All Upfront, Partial Upfront, and NoUpfront.
...
### AWS Cost Explorer
-  It is a free tool that allows you to **view charts** of your costs. You can view cost data for the **past 13 months** and **forecast** how much you will likely spend over the **next three months**. Cost Explorer can be used to **discover patterns** in how much you spend on AWS resources over time and to **identify the cost problem** area.
...
- It **converts video and audio files** from their source format into versions that will **playback** on devices like smartphones, tablets and PCs.
...
-  With the below options, organizations can **reduce their cost**:
...
- It is your **go-to**, the central resource for compliance-related information that matters to you.
...
### Amazon Neptune
- Amazon Neptune is a fast, reliable, fully-managed **graph database** service that makes it easy to build and run applications with highly connected datasets.
...
### Amazon Detective
- It uses **machine learning** and **graph theory** capability to collect log data to help you conduct faster and more efficient security investigations.

### Amazon Global Accelerator
- You are asked to **improve the performance** of the application for <ins>local and global users</ins>. As part of this initiative, you'll need** to monitor** the **application endpoint health** and **route traffic** to the most appropriate endpoint. To achieve this, we should use Amazon Global Accelerator.
...
- For implementing a threat detection service that continuously monitors malicious activities and **unauthorized behaviours** to protect AWS accounts, workloads and data stored in Amazon S3, we use this service.
...
***
### Note
- **Loose Coupling**: A desirable attribute of an IT system is that it can be broken into smaller, loosely coupled components.
...
- **Bootstrapping** and **Infrastructure as code** are two techniques for using automated, repeatable processes that are fast and avoid human error.
...
- "**virtual gateway**" on the <ins>VPC side</ins> and a **customer gateway** on <ins>the on-premise network side</ins> are needed to connect VPC with a VPN connection (Those are <ins>parts of Amazon Managed VPN connection</ins>).
...
- Your payment model in the cloud is operational (**OPEX**)
...
- With "**EC2**, **Auto Scaling** and **E**lastic **L**oad **B**alancing" combination of AWS services could deploy a **stateless** web application that can automatically and elastically scale.
- With the AWS cloud, you get **fine-grained** billing and can **turn off resources** you are not using easily and do not have to pay for them.
- To install a **PCI-compliant** workload on AWS:
	-  Use an AWS service in scope for PCI compliance and apply PCI controls at the application layer.
-  In IAM user access and security keys:
